...
class GlobalForecastModule(LightningModule):
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        #path = "/data/data_WF/ERA5/predict_24_2/"
        #base_date = datetime.datetime.strptime('2022010100', "%Y%m%d%H")
        #curr_date  = base_date + datetime.timedelta(hours=(batch_idx + 3))
        #preds  = preds.to('cpu').numpy()
        #numpy.save(path + curr_date.strftime("%Y%m%d%H") + ".npy",preds)
        all_loss_dicts = self.net.evaluate(
            x,
            y,
            lead_times,
